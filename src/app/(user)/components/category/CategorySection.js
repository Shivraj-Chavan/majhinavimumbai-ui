...
export default function CategoryCards() {
  ...
  const SkeletonCard = () => (
    <div className="flex flex-col items-center justify-center w-full max-w-[100px] sm:max-w-[120px] md:max-w-[160px] p-2 bg-white rounded-lg shadow-sm animate-pulse">
      <div className="w-full h-[80px] sm:h-[100px] md:h-[120px] rounded-xl bg-gray-300 border border-gray-300" />
      <div className="mt-2 h-4 w-16 sm:w-20 md:w-28 bg-gray-300 rounded" />
    </div>
  );
  ...
  return (
    <div id="categories" className="p-2 sm:p-4 max-w-6xl xl:max-w-7xl mx-auto mt-6">
      <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 xl:grid-cols-6 gap-3 sm:gap-4 font-poppins">
        {loading ?  Array.from({ length: 19 }).map((_, idx) => <SkeletonCard key={idx} />)
